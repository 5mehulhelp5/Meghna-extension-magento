...
{
    public function __construct(private readonly ScopeConfigInterface $scopeConfig, private readonly LoyaltyLedgerRepositoryInterface $ledgerRepository, private readonly LoyaltyLedgerInterfaceFactory $ledgerFactory, private readonly SearchCriteriaBuilderFactory $searchCriteriaBuilderFactory, private readonly SortOrderBuilder $sortOrderBuilder, private readonly WalletLogger $logger)
    {
    }
    ...
    public function execute(Observer $observer): void
    {
        try {
            ...
            $rate = (float)$this->scopeConfig->getValue('codilar_loyalty/general/points_per_currency', ScopeInterface::SCOPE_STORE, $storeId);

            $pointsEarned = (int)floor($subtotal * $rate);

            if ($pointsEarned <= 0) {
                $this->logger->info("Loyalty: Skipped. Calculated points <= 0. " . "Subtotal: {$subtotal}, Rate: {$rate}");
                return;
            }
            ...
            $searchCriteria = $this->searchCriteriaBuilderFactory->create()->addFilter('customer_id', $customerId)->addSortOrder($sortOrder)->setPageSize(1)->create();
            ...
            $ledger->setOrderIncrementId((string)$order->getIncrementId());
