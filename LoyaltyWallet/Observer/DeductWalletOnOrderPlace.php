...
class DeductWalletOnOrderPlace implements ObserverInterface
{
    ...
    public function execute(Observer $observer): void
    {
        try {
            ...
            if (!$order) {
                $this->logger->error(
                    'WALLET DEDUCTION: Order is missing from event.'
                );
            ...
            $walletAmount = (float)$order->getData(
                'applied_wallet_amount'
            );

            $this->logger->info(
                'WALLET DEDUCTION STARTED'
                . ' | Order ID: ' . $orderId
            ...
            if ($walletAmount <= 0) {
                $this->logger->info(
                    'WALLET DEDUCTION SKIPPED: Wallet amount is zero.'
                );
            ...
            if ($customerId <= 0) {
                $this->logger->info(
                    'WALLET DEDUCTION SKIPPED: Customer ID is invalid.'
                );
             ...
             */
            $comment = 'Wallet amount deducted from purchase - Order #'
                . $incrementId;
            ...
            $this->logger->error(
                'WALLET DEDUCTION ERROR: ' . $e->getMessage()
            );
        }
    }
}
